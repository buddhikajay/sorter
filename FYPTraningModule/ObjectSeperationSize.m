function [k,outFile] = ObjectSeperationSize(folderLoaction, Option, sizeTh,ROIX,ROIY)
...
for ii = 1:nfiles
    ...
    for idy = 2:length(B)
        boundary = B{idy};
        if  (min(boundary(:,1))>0)&(max(boundary(:,1))<ROIX)&(min(boundary(:,2))>0)&(max(boundary(:,2))<ROIY)
        imCroped = imRGB(...
